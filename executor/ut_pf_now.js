...
WScript.Echo("=== current split 150/60 (71.4/28.6) vs tested weights (fee 0.1%) ===");
var SPLIT = [[0.714, 0.286, "150/60 CURRENT"], [0.5, 0.5, "50/50"], [0.44, 0.56, "44/56 planned"], [0.6, 0.4, "60/40"]];
for (var v = 0; v < SPLIT.length; v++) {
  var a = comboRun([UB, RS], [SPLIT[v][0], SPLIT[v][1]], 0.001, STARTC, null);
  var b = comboRun([UB, RS], [SPLIT[v][0], SPLIT[v][1]], 0.001, "2024-09-13", null);
  WScript.Echo("  UB/ROT " + SPLIT[v][2] + ": full CAGR=" + (a.cagr * 100).toFixed(0) + "% DD=" + (a.dd * 100).toFixed(0) + "% Calmar=" + a.cal.toFixed(2) + " | OOS Calmar=" + b.cal.toFixed(2));
}
WScript.Echo("");
WScript.Echo("=== 3-leg turtle pick: SOL vs ETH version (fee 0.1%) ===");
var T3 = [[[TS, UB, RS], [0.4, 0.3, 0.3], "SOL-turtle 40/30/30"], [[TE, UB, RS], [0.4, 0.3, 0.3], "ETH-turtle 40/30/30"], [[TS, UB, RS], [0.3, 0.4, 0.3], "SOL-turtle 30/40/30"], [[TS, UB, RS], [0.25, 0.5, 0.25], "SOL-turtle 25/50/25"]];
for (var u = 0; u < T3.length; u++) {
  var c1 = comboRun(T3[u][0], T3[u][1], 0.001, STARTC, null);
  var c2 = comboRun(T3[u][0], T3[u][1], 0.001, "2024-09-13", null);
  WScript.Echo("  " + T3[u][2] + ": full CAGR=" + (c1.cagr * 100).toFixed(0) + "% DD=" + (c1.dd * 100).toFixed(0) + "% Calmar=" + c1.cal.toFixed(2) + " | OOS CAGR=" + (c2.cagr * 100).toFixed(0) + "% DD=" + (c2.dd * 100).toFixed(0) + "% Calmar=" + c2.cal.toFixed(2));
}
