...
WScript.Echo("=== [A] rebalance cost stress (fee on monthly turnover) ===");
var fees = [0, 0.001, 0.002];
for (var f2 = 0; f2 < fees.length; f2++) {
  var r4 = comboRun(W4, WS4, fees[f2], STARTC, null);
  var r2 = comboRun(W2, WS2, fees[f2], STARTC, null);
  WScript.Echo("  fee=" + (fees[f2] * 100).toFixed(2) + "%/turn: 4LEG CAGR=" + (r4.cagr * 100).toFixed(0) + "% DD=" + (r4.dd * 100).toFixed(0) + "% Calmar=" + r4.cal.toFixed(2) +
    " | 2LEG(live) CAGR=" + (r2.cagr * 100).toFixed(0) + "% DD=" + (r2.dd * 100).toFixed(0) + "% Calmar=" + r2.cal.toFixed(2));
}
WScript.Echo("");
WScript.Echo("=== [B] extreme-event windows (4LEG, fee 0.1%) ===");
function evt(name, fromD, toD) {
  var r = comboRun(W4, WS4, 0.001, fromD, toD);
  var pk = -1, dd = 0, ret = r.curve[r.curve.length - 1].eq / r.curve[0].eq - 1;
  for (var q = 0; q < r.curve.length; q++) {
    if (r.curve[q].eq > pk) pk = r.curve[q].eq;
    var d3 = (pk - r.curve[q].eq) / pk;
    if (d3 > dd) dd = d3;
  }
  WScript.Echo("  " + name + " (" + fromD + ".." + toD + "): ret=" + (ret >= 0 ? "+" : "") + (ret * 100).toFixed(1) + "% intraDD=" + (dd * 100).toFixed(1) + "%");
}
evt("FTX collapse   ", "2022-11-06", "2022-11-15");
evt("squeeze 2024-03", "2024-02-28", "2024-03-20");
evt("carry 2024-08  ", "2024-08-02", "2024-08-13");
evt("2026-09 crash  ", "2026-09-10", "2026-09-16");
// worst 7d rolling
(function () {
  var r = comboRun(W4, WS4, 0.001, STARTC, null);
  var worst = [];
  for (var q2 = 7; q2 < r.curve.length; q2++) {
    var w = r.curve[q2].eq / r.curve[q2 - 7].eq - 1;
    worst.push([w, r.curve[q2].d]);
  }
  worst.sort(function (a, b) { return a[0] - b[0]; });
  WScript.Echo("  worst-5 rolling 7d (4LEG):");
  for (var z = 0; z < 5; z++) WScript.Echo("    " + worst[z][1] + " : " + (worst[z][0] * 100).toFixed(1) + "%");
})();
// correlation spike in FTX window (±30d)
(function () {
  function corrW(m1, m2, fromD, toD) {
    var ds = [];
    for (var q3 = 0; q3 < cdt.length; q3++) if (cdt[q3] >= fromD && cdt[q3] <= toD) ds.push(cdt[q3]);
    if (ds.length < 10) return NaN;
    var a = [], b = [];
    for (var q4 = 1; q4 < ds.length; q4++) { a.push(m1[ds[q4]] / m1[ds[q4 - 1]] - 1); b.push(m2[ds[q4]] / m2[ds[q4 - 1]] - 1); }
    var n = a.length, ma = 0, mb = 0;
    for (var i5 = 0; i5 < n; i5++) { ma += a[i5]; mb += b[i5]; }
    ma /= n; mb /= n;
    var num = 0, va = 0, vb = 0;
    for (var j3 = 0; j3 < n; j3++) { num += (a[j3] - ma) * (b[j3] - mb); va += (a[j3] - ma) * (a[j3] - ma); vb += (b[j3] - mb) * (b[j3] - mb); }
    return num / Math.sqrt(va * vb);
  }
  WScript.Echo("  corr in FTX window (22-10-07..22-12-07) vs normal(common):");
  WScript.Echo("    UB~TS: " + corrW(UB, TS, "2022-10-07", "2022-12-07").toFixed(2) + " | UB~RS: " + corrW(UB, RS, "2022-10-07", "2022-12-07").toFixed(2) +
    " | TS~RS: " + corrW(TS, RS, "2022-10-07", "2022-12-07").toFixed(2));
})();
WScript.Echo("");
WScript.Echo("=== [C] SOL-legacy-free windows (fee 0.1%) ===");
var wins = [["2023-12-01", "from 2024-01"], ["2024-09-13", "OOS 2y"]];
for (var w2 = 0; w2 < wins.length; w2++) {
  var a4 = comboRun(W4, WS4, 0.001, wins[w2][0], null);
  var b4 = comboRun(W2, WS2, 0.001, wins[w2][0], null);
  var c4 = comboRun(W3S, WS3S, 0.001, wins[w2][0], null);
  WScript.Echo("  " + wins[w2][1] + ": 4LEG CAGR=" + (a4.cagr * 100).toFixed(0) + "% DD=" + (a4.dd * 100).toFixed(0) + "% Calmar=" + a4.cal.toFixed(2) +
    " | 3LEG(SOL) Calmar=" + c4.cal.toFixed(2) + " | 2LEG(live) Calmar=" + b4.cal.toFixed(2));
}
