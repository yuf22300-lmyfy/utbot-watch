...
(function () {
  var ds = [];
  for (var k6 in UB) ds.push(k6);
  ds.sort();
  var e = 100;
  UB2[ds[0]] = 100;
  for (var i6 = 1; i6 < ds.length; i6++) { e *= (1 + 2 * (UB[ds[i6]] / UB[ds[i6 - 1]] - 1)); UB2[ds[i6]] = e; }
})();
(function () {
  var r = comboRun([UB2, RS], [0.5, 0.5], 0.001, STARTC, null);
  var worst = [];
  ...
  WScript.Echo("UB2x/ROT 50/50 worst-5 rolling 7d:");
  for (var z = 0; z < 5; z++) WScript.Echo("  " + worst[z][1] + " : " + (worst[z][0] * 100).toFixed(1) + "%");
  var r2 = comboRun([UB2, RS], [0.714, 0.286], 0.001, STARTC, null);
  var worst2 = [];
  for (var q3 = 7; q3 < r2.curve.length; q3++) worst2.push([r2.curve[q3].eq / r2.curve[q3 - 7].eq - 1, r2.curve[q3].d]);
  worst2.sort(function (a, b) { return a[0] - b[0]; });
  WScript.Echo("UB2x/ROT 150/60 (current) worst-5 rolling 7d:");
  for (var z2 = 0; z2 < 5; z2++) WScript.Echo("  " + worst2[z2][1] + " : " + (worst2[z2][0] * 100).toFixed(1) + "%");
})();
